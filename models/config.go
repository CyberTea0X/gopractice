...
type DatabaseConfig struct {
	...
	SSLMode  string `toml:"sslmode" validate:"required"`
}

func (c *DatabaseConfig) GetUrl() string {
	return fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?charset=utf8&parseTime=True&loc=Local", c.User, c.Password, c.Host, c.Port, c.Database)
}

type SmsCodesConfig struct {
	LifespanCfg `validate:"required"`
}
...
type TomlConfig struct {
	...
	SmsCodes SmsCodesConfig `toml:"sms_codes" validate:"required"`
}
